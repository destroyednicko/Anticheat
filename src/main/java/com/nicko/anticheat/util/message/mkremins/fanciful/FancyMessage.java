...
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	 ...
	 */
	public FancyMessage color(final CC color) {
		latest().color = color;
		dirty = true;
		return this;
	}
	...
	*/
	...
	public FancyMessage append(final Object text) {
		return then(TextualComponent.rawText((String) text));
	}
		...
		for (JsonElement mPrt : extra) {
			...
			for (Map.Entry<String, JsonElement> entry : messagePart.entrySet()) {
				...
				if (TextualComponent.isTextKey(entry.getKey())) {
					...
					component.color = CC.valueOf(entry.getValue().getAsString().toUpperCase());
					...
					if (object.get("value").isJsonPrimitive()) {
						...
						// The only composite action we currently store is another FancyMessage
					...
					for (JsonElement object : entry.getValue().getAsJsonArray()) {
						if (object.isJsonPrimitive()) {
							...
							// Only composite action stored in this array is - again - FancyMessages
