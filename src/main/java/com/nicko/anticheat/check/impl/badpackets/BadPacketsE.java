...
public class BadPacketsE extends PacketCheck {
    ...
    public BadPacketsE(AnticheatPlugin plugin, PlayerData playerData) {
        super(plugin, playerData, "Bad Packets§7[§cE§7]");
    }
    ...
    public void run(Player player, Packet packet) {
        if (packet instanceof PacketPlayInEntityAction) {
            ...
            if (playerAction == PacketPlayInEntityAction.EnumPlayerAction.START_SNEAKING ||
                    playerAction == PacketPlayInEntityAction.EnumPlayerAction.STOP_SNEAKING) {
                if (this.sentAction) {
